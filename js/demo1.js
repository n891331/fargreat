createLandscape({
  ...
  palletehImage:'img/palleteh.png'
})
...
function createLandscape(params){
  ...
  function sceneTextures(){
    // pallete
    new THREE.TextureLoader().load( params.palletehImage, function(texture){
      terrain.material.uniforms.palleteh.value = texture;
  ...
  function sky(){
    ...
    // sky.material.uniforms.rayleigh.value = 1.119;
    // sky.material.uniforms.rayleigh.value = 0.729;
    ...
    // sky.material.uniforms.mieCoefficient.value = 0.056;
    ...
    // 
    // sky.material.uniforms.inclination.value = 0;
    // sky.material.uniforms.azimuth.value = 0.2472;
...
function animateTitles() {
  ...
  const glitch = (el,cycles) => {
    ...
    TweenMax.set(el, {
      ...
      color: ['#4716ED','#00ffff','#ff5338'][cycles-1]
    });
    setTimeout(() => {
